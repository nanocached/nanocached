...
# compress and decompress side. See value compression for why raw DEFLATE
...
class DecompressionError(NanocachedError):
    ...
    clients sharing this key (value compression's compatibility caveat:
    ...
    if marker == _MARKER_DEFLATE:
        try:
            ...
            raise DecompressionError(
                ...
                f"did every client sharing this key enable compress (value compression)? ({error})"
            ) from error
